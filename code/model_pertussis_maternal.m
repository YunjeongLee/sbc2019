...
dydt(1:num_grps_) = ...
    ...
    - [B_ * f_A_; zeros(num_grps_-1, 1)] ...
    + [0; c_(1) * (1 - vacc_(2)) * y(9*num_grps_ + 1); zeros(num_grps_-2,1)];

%% Generate partially vaccinated group 1 (P1_AI)
dydt(num_grps_+1:2*num_grps_) = ...
    - p1_ * Lambda .* y(num_grps_+1:2*num_grps_) ...
    - sigma0_ * y(num_grps_+1:2*num_grps_) ...
    ...
    + [0; c_(1) * vacc_(2) * y(9*num_grps_ + 1); zeros(num_grps_-2, 1)];

%% Generate partially vaccinated group 2 (P2_AI)
dydt(2*num_grps_+1:3*num_grps_) = ...
    - p2_ * Lambda .* y(2*num_grps_+1:3*num_grps_) ...
    - tauP_ * y(2*num_grps_+1:3*num_grps_) ...
...
dydt(3*num_grps_+1:4*num_grps_) = ...
    - p3_ * Lambda .* y(3*num_grps_+1:4*num_grps_) ...
    - tauP_ * y(3*num_grps_+1:4*num_grps_) ...
...
dydt(6*num_grps_+1:7*num_grps_) = ...
    ...
    + [p1_ * Lambda(1) * y(9*num_grps_ + 1); zeros(num_grps_-1, 1)];

%% Generate mild infectious group (I3)
...
%% Generate maternally immunized group (X0)
dydt(9*num_grps_ + 1) = ...
    - p1_ * Lambda(1) * y(9*num_grps_ + 1) - mu_(1) * y(9*num_grps_ + 1) ...
